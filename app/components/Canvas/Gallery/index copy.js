...
export default class Gallery extends Component {
	...
	createItems() {
		return new Promise((res) => {
			this.getBounds().then((imageBounds) => {
				...
				const mainImagePromises = Object.values(this.mainImages).map(
					(img) =>
						new Promise((resolve) => {
		...
		if (this.imageBounds) {
			...
			this.getBounds().then(() => {
				...
				if (this.mainItems[0] && this.wBounds) {
					// Refresh currentMainBounds from the newly-read wBounds/hBounds — the old
					// DOMRect reference becomes stale after getBoundingClientRect() re-runs.
					if (this.currentMainBounds && this.previous) {
						this.currentMainBounds = this.previous.dataW == 1818
							? this.wBounds
							: this.hBounds;
					}
	...
	_updateMainOverlays() {
		...
		if (item.cornerMeshes) {
			...
			item.cornerMeshes.forEach((m, i) => {
				m.scale.set(cornerSize, cornerSize, 1);
				m.position.set(mx + offsets[i].dx, my + offsets[i].dy, -1.4);
			});
		}
	}
